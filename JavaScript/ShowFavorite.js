let archive = [];
// to get all favorites from localStorage
function allStorage() {
...
// to show pokemons favorites on Popup modal 
async function ShowFavoriteBtn() {
    ...
    if (archive[0] != undefined) {
        ...
        archive.map(async function (id) {
            ...
            favoritesBtn.innerHTML = `<button id="but" onclick="RemoveFromFavoriteOnFav(${id});" >Remove From Favorite</button>`;
...
function RemoveFromFavoriteOnFav(Pokeid) {
    alert(Pokeid)
